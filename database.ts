...
export const saveMember = async (member: Member): Promise<void> => {
  if (!isBrowser) {
    // Fallback to localStorage if not in browser
    const { saveMember: saveMemberFallback } = await import('./localStorage')
    saveMemberFallback(member)
    return
  }
  ...
  try {
    ...
    if (checkError && checkError.code !== 'PGRST116') {
      // PGRST116 is "not found" which is fine for new members
      throw checkError
    }
    ...
    console.error('Supabase member save failed, falling back to localStorage:', error)
  ...
  try {
    ...
    // If both fail, we still don't throw - the UI will handle it
  }
  
  // Only throw if we want to signal an error to the UI
  // But since we have localStorage fallback, we don't need to throw
  if (!supabaseSuccess) {
    // Don't throw - localStorage fallback succeeded
    // The UI will show a message that it was saved locally
  }
}
...
export const deleteMember = async (memberId: string): Promise<void> => {
  if (!isBrowser) {
    // Fallback to localStorage if not in browser
    const { deleteMember: deleteMemberFallback } = await import('./localStorage')
    deleteMemberFallback(memberId)
    return
  }
  ...
  try {
    ...
    console.error('Supabase member delete failed, falling back to localStorage:', error)
...
export const saveSettings = async (settings: GymSettings): Promise<void> => {
  if (!isBrowser) {
    // Fallback to localStorage if not in browser
    const { saveSettings: saveSettingsFallback } = await import('./localStorage')
